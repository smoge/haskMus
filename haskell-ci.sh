...
# REGENDATA ("0.17.20231010",["bash","haskMus.cabal"])
...
PKGDIR_haskMus="$(find "$BUILDDIR/unpacked" -maxdepth 1 -type d -regex '.*/haskMus-[0-9.]*')"
run_cmd touch cabal.project
run_cmd touch cabal.project.local
echo_to cabal.project "packages: ${PKGDIR_haskMus}"
echo_to cabal.project "package haskMus"
echo_to cabal.project "    ghc-options: -Werror=missing-methods"
cat >> cabal.project <<EOF
EOF
$HCPKG list --simple-output --names-only | perl -ne 'for (split /\s+/) { print "constraints: $_ installed\n" unless /^(haskMus)$/; }' >> cabal.project.local
run_cmd cat cabal.project
