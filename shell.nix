...
let
  ...
  f = { mkDerivation, base, containers, data-default
      ...
      , dlist, doctest, fgl, hspec, HUnit, lens, lib, linear, mtl, parsec
      , pretty-simple, QuickCheck, random-shuffle, template-haskell
      , test-framework, test-framework-hunit, test-framework-quickcheck2
      , text, vector-space, xml-conduit, xml-types
      }:
      mkDerivation {
        pname = "haskMus";
        version = "0.1.0.0";
        ...
        libraryHaskellDepends = [
          ...
          data-default-instances-old-locale dlist doctest fgl hspec HUnit
          lens linear mtl parsec pretty-simple QuickCheck random-shuffle
          template-haskell text vector-space xml-conduit xml-types
        ];
