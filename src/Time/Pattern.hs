...
instance IsPattern Pattern where
    ...
    asStream (PBind paramStreams) = Stream $ \t -> do
        ...
        return (event, asStream (PBind paramStreams'))
...
-- Utility functions to create different types of patterns
    ...
    runPattern steps p = go steps (asStream p) 0
      where
        ...
        go n s t = do
          (x, s') <- runStream s t
          events <- go (n-1) s' t
          return (Event t 0 (Map.singleton "value" x) : events)
...
-- | Main function demonstrating the use of the pattern system.
main :: IO ()
main = do
    -- Define patterns for duration, frequency, and amplitude
    let durPattern = pwhite 0.00001 0.5  -- Random durations between 0.00001 and 0.5 seconds
        freqPattern = pgeom 100 1.05 12  -- Geometric series of frequencies, starting at 100 Hz
        ampPattern = pwhite 0.1 10.8     -- Random amplitudes between 0.1 and 10.8
